...
SamplerState* D3D10SamplerState::CreateSamplerState(SamplerInfo &info)
{
    ...
    ID3D10Device1 *d3d=static_cast<ID3D10Device1 *>(GetD3D());
...
Texture* D3D10Texture::CreateFromSharedHandle(unsigned int width, unsigned int height, HANDLE handle)
{
    ...
    ID3D10Device1 *d3d=static_cast<ID3D10Device1 *>(GetD3D());
    if(FAILED(err = d3d->OpenSharedResource(handle, __uuidof(ID3D10Resource), (void**)&tempResource)))
    {
...
Texture* D3D10Texture::CreateTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat, void *lpData, BOOL bGenMipMaps, BOOL bStatic)
{
   ...
   ID3D10Texture2D *texVal = nullptr;
...
Texture* D3D10Texture::CreateFromFile(CTSTR lpFile, BOOL bBuildMipMaps)
{
    ...
    ID3D10Device1 *d3d=static_cast<ID3D10Device1 *>(GetD3D());
    if(FAILED(err = D3DX10CreateTextureFromFile(d3d, lpFile, &ili, NULL, &texResource, NULL)))
    {
...
Texture* D3D10Texture::CreateReadTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat)
{
   HRESULT err;

   if (colorFormat >= GS_DXT1) {
      AppWarning(TEXT("D3D10Texture::CreateReadTexture: tried to create a blank DXT texture.  Use CreateFromFile instead."));
      return NULL;
   }
...
D3D10Texture::D3D10Texture():Texture()
{
   mIsReadOnly=false;
}
...
bool D3D10Texture::GetDC(HDC &hDC)
{
    ...
    if (texture == nullptr) {
        AppWarning(TEXT("D3D10Texture::GetDC: texture is nullptr"));
        return false;
    }
...
void D3D10Texture::SetImage(void *lpData, GSImageFormat imageFormat, UINT pitch)
{
    ...
    if (texture == nullptr) {
        AppWarning(TEXT("D3D10Texture::SetImage: texture is nullptr"));
        return;
    }
...
bool D3D10Texture::Map(BYTE *&lpData, UINT &pitch)
{
    ...
    if (texture == nullptr) {
        AppWarning(TEXT("D3D10Texture::Map: texture is nullptr"));
        return false;
    }

    if (FAILED(err = texture->Map(0, mapType, 0, &map)))
    {
        ...
        //We have to crash, or we end up deadlocking the thread when the convert threads are never signalled
         if (err == DXGI_ERROR_DEVICE_REMOVED) {
          ...
          HRESULT reason = (HRESULT)OBSGraphics_GetDeviceRemovedReason();
          ...
          OBSApiLog(" D3D10Texture Texture->Map failed: 0x%08x 0x%08x\r\n", err, reason);
       }
       else
       {
         //CrashError(TEXT("Texture->Map failed: 0x%08x"), err);
         OBSApiLog("D3D10Texture Texture->Map failed: 0x%08x", err);
       }
       return false;
    }
...
HANDLE D3D10Texture::GetSharedHandle()
{
    ...
    if (texture == nullptr) {
        AppWarning(TEXT("D3D10Texture::GetSharedHandle: texture is nullptr"));
        return handle;
    }

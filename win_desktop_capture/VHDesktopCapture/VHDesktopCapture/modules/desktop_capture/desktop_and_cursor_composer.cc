...
void DesktopAndCursorComposer::OnCaptureResult(
    ...
    std::unique_ptr<DesktopFrame> frame) {
  if (!frame ||DesktopCapturer::Result::SUCCESS != result) {
    return;
  }
  /* copy from shared mem */
  std::unique_ptr<DesktopFrame> blendFrame;
  blendFrame.reset(new BasicDesktopFrame(frame->size()));
  memcpy(blendFrame->data(), frame->data(), frame->stride() * frame->size().height());
  blendFrame->CopyFrameInfoFrom(*frame);

  if (blendFrame && cursor_) {
    if (use_desktop_relative_cursor_position_) {
      if (blendFrame->rect().Contains(cursor_position_) &&
        ...
        blendFrame = rtc::MakeUnique<DesktopFrameWithCursor>(
            std::move(blendFrame), *cursor_, relative_position);
      }
    } else {
      if (cursor_state_ == MouseCursorMonitor::INSIDE) {
        blendFrame = rtc::MakeUnique<DesktopFrameWithCursor>(
            std::move(blendFrame), *cursor_, cursor_position_);
      }
    }
  }

  callback_->OnCaptureResult(result, std::move(blendFrame));
}

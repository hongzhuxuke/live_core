...
bool VHEnumWindows(DesktopCapturer::SourceList* list){

	HWND window = GetWindow(GetDesktopWindow(), GW_CHILD);

	while (window) {
		if (WindowValid(window) && GetParent(window) == NULL) {
			DesktopCapturer::Source windowItem;
			windowItem.id = reinterpret_cast<WindowId>(window);
			const size_t kTitleLength = 500;
			WCHAR window_title[kTitleLength];
			// Truncate the title if it's longer than kTitleLength.
			GetWindowText(window, window_title, kTitleLength);
			windowItem.title = rtc::ToUtf8(window_title);
			// Skip windows when we failed to convert the title or it is empty.
            if (!windowItem.title.empty()) {
                RECT clientRect;
                GetWindowRect(window, &clientRect);
                if (clientRect.right - clientRect.left > 0 &&
                    clientRect.bottom - clientRect.top > 0) {
                    list->push_back(windowItem);
                }
            }
		}

...
struct DShowInput {
   ...
   {
      bool block = true/*obs_data_get_bool(settings, "synchronous_activate")*/;
      ...
      : /*source(source_),*/ device(InitGraph::False)
   {
      ...
      deactivateWhenNotShowing = true;//obs_data_get_bool(settings, DEACTIVATE_WNS);
   ...
   inline enum video_colorspace GetColorSpace() const;
   inline enum video_range_type GetColorRange() const;
   inline bool Activate();
   inline void Deactivate();

   inline void SetupBuffering();
   ...
   long long endTime, long rotation)
{
   ...
   //obs_source_output_video2(source, &frame);
   if (videoConfig.recvVideoCallback) {
      videoConfig.recvVideoCallback(config,data,size, startTime,endTime, rotation) ;
   }
...
struct PropertiesData {
   ...
   bool GetDevice(VideoDevice &device, DeviceId deviceId) const
   {
      //DeviceId deviceId;
...
{
   return sscanf_s(res, "%dx%d", &cx, &cy) == 2;
}
...
{
   videoConfig = InputVideoConfig;
   //string video_device_id = InputVideoConfig.path;
   deactivateWhenNotShowing = true;
   flip = false;/*obs_data_get_bool(settings, FLIP_IMAGE);*/

   DeviceId id;

   id.path = InputVideoConfig.path;
   id.name = InputVideoConfig.name;

   PropertiesData data;
   Device::EnumVideoDevices(data.devices);
   VideoDevice dev;
   if (!data.GetDevice(dev, id)) {
      return false;
   }

   int resType = ResType_Custom;
   int cx = 0, cy = 0;
   long long interval = 0;
   VideoFormat format = VideoFormat::Any;

   bool has_autosel_val;
   cx = InputVideoConfig.cx;
   cy = InputVideoConfig.cy_abs;
   interval = InputVideoConfig.frameInterval;

   format = (VideoFormat)InputVideoConfig.format;
...
bool DShowInput::UpdateAudioConfig()
{
   ...
   audioConfig.useVideoDevice = !deviceHasSeparateAudioFilter;
...
inline bool DShowInput::Activate(/*obs_data_t *settings*/)
{
   ...
   enum video_colorspace cs = GetColorSpace();
   range = GetColorRange();
   frame.range = range;
...
static void *CreateDShowInput()
{
   DShowInput *dshow = nullptr;
   dshow = new DShowInput();
   return dshow;
}
...
static void UpdateDShowInput(void *data)
{
...
//static bool SetResolution(string res, bool autoselect = false)
//{
//	if (autoselect)
...
void CaptureEngineImpl::GetVideoDevice(std::vector<DShow::VideoDevice> &devices) {
   DShow::Device::EnumVideoDevices(devices);
}

void CaptureEngineImpl::GetAudioDevice(std::vector<DShow::AudioDevice> &devices) {
   DShow::Device::EnumAudioDevices(devices);
}


int CaptureEngineImpl::CreateInputDevice(DShow::DeviceInputType input_type) {
   mInput = new DShowInput();
   return 0;
}

int CaptureEngineImpl::StartCaptureDevice(DShow::VideoConfig &video_config) {
   mVideoConfig = video_config;
   if (mInput) {                                                                                      
      //mInput->QueueAction(Action::ConfigVideo);
      mInput->InputVideoConfig = video_config;
      mInput->SetActive(true);
   }
   return 0;
}

int CaptureEngineImpl::SetCaptureDevice(DShow::AudioConfig &audio_config) {
   mAudioConfig = audio_config;
   if (mInput) {
      //mInput->QueueAction(Action::ConfigVideo);
      mInput->audioConfig = audio_config;
      mInput->SetActive(true);
   }
   return 0;
}

int CaptureEngineImpl::StartCapture() {
   if (mInput) {
      mInput->SetActive(true);
   }
   return 0;
...
int CaptureEngineImpl::StopCapture() {
   if (mInput) {
...
namespace vlive {
   VH_IDSENGINE_EXPORT IDShowEngine* CreateEngine() {
      IDShowEngine* obj = new CaptureEngineImpl();
       return obj;
   }

   VH_IDSENGINE_EXPORT void DestoryEngine(IDShowEngine* obj) {
      if (obj) {
         delete obj;
         obj = nullptr;
      }

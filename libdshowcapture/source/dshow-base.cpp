...
void LogFilters(IGraphBuilder *graph)
{
	...
	HRESULT hr;
   OutputDebugString(L"=======================[LogFilters]===================\n");
   if (graph == NULL) {
      return;
   }
	hr = graph->EnumFilters(&filterEnum);
   if (FAILED(hr)) {
      OutputDebugString(L"===========LogFilters Failed=================\n");
   	return;
   }
...
static bool PinConfigHasMajorType(IPin *pin, const GUID &type)
{
	...
	for (int i = 0; i < count; i++) {
      ...
      if (SUCCEEDED(config->GetStreamCaps(i, &mt, caps.data())) && mt != NULL) {
